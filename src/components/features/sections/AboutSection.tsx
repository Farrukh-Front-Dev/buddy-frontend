...
import { Shield, Heart, MessageSquare, Zap, Users, Award } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <motion.div
          className="absolute top-1/2 left-1/4 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl opacity-20"
        ...
        >
          {missionCards.map((card, index) => (
            <motion.div key={index} variants={itemVariants}>
              <GlassCard hover accent={card.accent} className="p-8 h-full">
                <div
                  className={`w-14 h-14 rounded-xl bg-gradient-to-br ${card.color} p-3 mb-6 text-white`}
                >
                  {card.icon}
                </div>
